...
std::vector<std::string> Server::list_available_games() {
    std::vector<std::string> res;
    for (int i = 0; i < 16; i++) {
        if (root->games[i].used && root->games[i].is_public && root->games[i].state == GAME_WAITING) {
            std::string info = "🎮 " + std::string(root->games[i].game_name) + 
                             " (ID: " + std::to_string(i) + ") - создатель: " + 
                             std::string(root->games[i].player1);
            res.emplace_back(info);
        }
    }
    return res;
...
int Server::create_private_game(const std::string& creator, const std::string& target) {
    ...
    std::cout << "Private game '" << game_name << "' created by " << creator << " for " << target << std::endl;
    return game_id;
}
...
int Server::create_public_game(const std::string& game_name, const std::string& creator) {
    ...
    std::cout << "Public game '" << game_name << "' created by " << creator << std::endl;
    return game_id;
}
...
void Server::remove_game(int game_id) {
    ...
    if (it != games_map.end()) {
        Game* game = it->second;
        ClientSlot* client1 = find_client(game->get_player1().c_str());
        ClientSlot* client2 = find_client(game->get_player2().c_str());
...
bool Server::parse_ship_placement(const std::string& payload, uint8_t& size, 
...
void Server::handle_surrender(const Message &m) {
    ...
    std::string opponent = (game->get_player1() == m.from) ? 
                          game->get_player2() : game->get_player1();
...
void Server::handle_message(const Message &m) {
    ...
    switch (m.type) {
        ...
        case MSG_INVITE: {
            ...
            if (!tgt) {
                ...
                if (game_id == -1) {
                    ...
                    if (game) {
                        char buf[RESP_MAX];
                        std::snprintf(buf, RESP_MAX, "INVITE_FROM:%s:GAME:%s:ID:%d", 
                                     m.from, game->get_game_name().c_str(), game_id);
        ...
        case MSG_JOIN: {
            ...
            if (!game->is_waiting()) {
                send_response_to(m.from, "JOIN_FAIL:Игра уже началась или завершена");
                break;
            }
            ...
            send_response_to(game->get_player1().c_str(), "OPPONENT_JOINED:Игрок присоединился");
        ...
        case MSG_ACCEPT: {
            ...
            if (sscanf(m.payload, "%d", &game_id) == 1) {
                ...
                if (!game) {
                    ...
                    send_response_to(game->get_player1().c_str(), "OPPONENT_JOINED:Игрок принял приглашение");
        ...
        case MSG_PLACE_SHIP: {
            std::cout << "DEBUG: Received PLACE_SHIP from " << m.from 
        ...
        case MSG_SHOT: {
            ...
            if (!game->is_game_active()) {
                send_response_to(m.from, "ERROR:Game not active. Wait for both players to complete setup.");
                break;
            }
            ...
            std::string opponent = (game->get_player1() == shooter) ? 
                                  game->get_player2() : game->get_player1();
            ...
            std::snprintf(buf, RESP_MAX, "OPPONENT_SHOT:%s at %d,%d:%s", 
                         shooter.c_str(), x, y, hit ? "HIT" : "MISS");
            ...
            if (game->is_game_finished()) {
                ...
                std::string loser = (winner == game->get_player1()) ? 
                                   game->get_player2() : game->get_player1();
